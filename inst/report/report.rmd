...
if(ncol(ov_table) > 3){
    if("#identified features" %in% names(ov_table) && "#quantifiable features" %in% names(ov_table)){
        ov_data <- ov_table %>% dplyr::rename(ID=`#identified features`) %>%
            dplyr::rename(Quant=`#quantifiable features`) %>%
...
if("quant_median_ks" %in% names(final_res$basic_metrics)){
    kable(final_res$basic_metrics$quant_median_ks %>%
              rename(data_dist_similarity=raw_quant_median_ks,
                     scaled_data_dist_similarity=quant_median_ks),"html",escape = FALSE) %>%
        kable_styling(bootstrap_options = "striped", full_width = F) #%>%
        #row_spec(which.max(id_table[,3]), bold = T, color = "red")
}
```
...
if(!is.null(final_res$network_table)){
  ...
  kable_styling(bootstrap_options = "striped", full_width = F)%>%
    row_spec(which(final_res$network_table$cor$complex_ks >= sort(final_res$network_table$cor$complex_ks,decreasing = TRUE)[3]), bold = T, color = "red")
}
...
if("QC" %in% cv_table$class){
  ...
  show_cv_table <- cv_table %>% dplyr::filter(class=="QC") %>% dplyr::select(class,median_cv,dataSet)
  kable(show_cv_table, "html",digits = 3) %>%
  kable_styling(bootstrap_options = "striped", full_width = F)%>%
