...
The table below shows the number of identified proteins or genes for each data table. We take the proteins or genes filtered by 50% missing value as quantified proteins or genes.
...
To quantify the normalization effect, for each pair of sample, perform an AUROC test to quantify the ability of feature abundance to distinguish the two samples and then generate a score based on 1-2*abs(AUROC-0.5), which will be 0 to 1, higher the better (no systematic difference between the two samples). The final metric for each data table is the median of scores from all sample pairs.
...
if("QC" %in% cv_table$class){
  ...
  kable_styling(bootstrap_options = "striped", full_width = F)%>%
    row_spec(which.min(show_cv_table$median_CV), bold = T, color = "red")
}
