...
if(!is.null(final_res$basic_metrics$datasets_id_overla)){
  cat("Upset chart below shows overlap between proteins or genes identified in each data table. Numbers of identified proteins or genes shared between different combination of data tables are indicated in the top bar chart, and the specific combinations of data tables in containing those proteins or genes are indicated with solid points below the bar chart. Total identifications for each data table are indicated on the right as ‘Set size’.\n")
...
```{r results="asis",fig.align='center'}
if(n_batch >= 2){
  cat("<p>Greater batch effect is more likely to be present when a PC that explains a higher percentage of variance showing significant correlation with the batch covariate. Therefore, we use the 'Scaled PC regression' metric (pcRegscale), i.e. the total variance of PCs which correlate significantly with batch covariate (FDR<0.05) scaled by the total variance of",pcr_res$top_pc,"PCs, to quantify the batch effect:</p>\n")
...
When a RNA data table is available for comparison with protein data tables, the concordance between the protein data and RNA data can be used to assess data quality, and vice versa. Here, we evaluate gene- and sample-wise correlations between the protein and RNA data tables.

```{r results="asis",fig.align='center'}
if(!is.null(final_res$input_parameters$x2)){
  cat("## Gene-wise mRNA-protein correlation\n")
  cat("<p>The table below shows the number of genes with measurements (n) in each data table as well as the median Spearman correlation between mRNA and protein at gene-wise. The columns n5, n6, n7 and n8 show the number of genes with correlation greater than 0.5, 0.6, 0.7 and 0.8, respectively.</p>\n")
