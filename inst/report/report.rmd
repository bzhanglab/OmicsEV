...
#save(final_res,file="a.rda")
if(ncol(ov_table) > 3){
    if("# proteins (genes)" %in% names(ov_table) && "# proteins (genes) [50%]" %in% names(ov_table)){
        ov_data <- ov_table %>% dplyr::rename(ID=`# proteins (genes)`) %>%
            dplyr::rename(Quant=`# proteins (genes) [50%]`) %>%
            dplyr::mutate(ID=ID/max(ID),Quant=Quant/max(Quant))
    }else{
...
# Identification and quantification depth
...
## Protein or gene expression distribution - Boxplot
...
if("quant_median_ks" %in% names(final_res$basic_metrics)){
    kable(final_res$basic_metrics$quant_median_ks %>%
              rename(mean_statistic=raw_quant_median_ks,scaled_mean_statistic=quant_median_ks),"html",escape = FALSE) %>%
        kable_styling(bootstrap_options = "striped", full_width = F) #%>%
        #row_spec(which.max(id_table[,3]), bold = T, color = "red")
}
```
...
## Protein or gene expression distribution - Density plot
...
```{r results="asis",fig.align='center'}
if(n_batch >= 2){
  cat("## Batch effect evaluation based on principal component regression\n\n")
...
if(!is.null(final_res$network_table)){
  cat("## Correlation among protein complex members: within vs between protein complexes\n")
