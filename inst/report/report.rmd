...
if(!is.null(final_res$snr)){
  ...
  kable_styling(bootstrap_options = "striped", full_width = F)%>%
    row_spec(which(final_res$snr$table$snr <= sort(final_res$snr$table$snr,decreasing = FALSE)[3]), bold = T, color = "red")
...
if(!is.null(final_res$snr)){
  figs <- final_res$snr$mad_figs
  figs <- unlist(get_full_path(figs))
  for(i in 1:length(figs)){
    knitr::include_graphics(figs[i] %>% normalizePath)
    cat(names(figs)[i])
    cat("![](",figs[i],")")
