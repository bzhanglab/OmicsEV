...
run_omics_evaluation(data_dir = "datasets/",
                     ...
                     data_type = "protein",
                     x2 = "rna_gene_quantification.tsv",
                     class_color="class_color.tsv",
                     use_class=c("tumor"),
                     cpu=0)
```
...
Users can also perform the evaluation analysis through the shiny-based GUI 
implemented in `OmicsEV`. The GUI can be lauched using the following function:
...
```{r echo=FALSE}
knitr::include_graphics("./gui.png" %>% normalizePath)
```
